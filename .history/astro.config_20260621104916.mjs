    ...
    vite: {
      resolve: {
        alias: {
          ...
          // Local shims to prevent Rollup from resolving platform-specific Clerk imports when disabled
          '@clerk/astro': new URL('./src/shims/clerk/index.mjs', import.meta.url).pathname,
          '@clerk/astro/components': new URL('./src/shims/clerk/components.mjs', import.meta.url).pathname,
          '@clerk/astro/server': new URL('./src/shims/clerk/server.mjs', import.meta.url).pathname
        }
      },
      optimizeDeps: {
        exclude: ['@clerk/astro']
