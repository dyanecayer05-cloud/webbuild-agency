...
export default defineConfig(async () => {
  ...
  // Generally avoid pre-bundling @clerk/astro to stop Vite from statically analyzing its virtual imports.
  ...
  if (process.env.CLERK_PUBLISHABLE_KEY) {
    ...
    viteOptions.build = { rollupOptions: { external: ['cloudflare:workers', '@clerk/astro', 'virtual:@clerk/astro/config'] } };
  } else {
    // When Clerk is disabled, still avoid bundling cloudflare virtual import
    viteOptions.ssr = { external: ['virtual:@clerk/astro/config'] };
    viteOptions.build = { rollupOptions: { external: ['cloudflare:workers', 'virtual:@clerk/astro/config'] } };
  }
